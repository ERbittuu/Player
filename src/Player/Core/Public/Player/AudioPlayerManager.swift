...
open class AudioPlayerManager: NSObject {
	...
	public enum BufferStrategy {
		/// The player will wait until enough data is loaded to minimize stalling.
        /// This will take longer to laod in the beginning but will lead to a smoother playback.
        /// Sets `AVPlayer.automaticallyWaitsToMinimizeStalling` to `true`
	...
	open func play(_ audioTrack: AudioTrack) {
		playerLog("play(audioTrack: \(audioTrack))")
		self.stop()
		self.play([audioTrack], at: 0)
	}

	open func play(_ audioTracks: [AudioTrack], at startIndex: Int) {
		playerLog("play(audioTracks: \(audioTracks.count) tracks, startIndex: \(startIndex))")
	...
	open func replace(with audioTrack: AudioTrack) {
		playerLog("replace(audioTrack: \(audioTrack))")
		self.replace(with: [audioTrack], at: 0)
	}

	open func replace(with audioTracks: [AudioTrack], at startIndex: Int) {
		playerLog("replace(audioTracks: \(audioTracks.count) tracks, startIndex: \(startIndex))")
		self.stop()
	...
	open func prepend(_ audioTracks: [AudioTrack], toQueue generation: Int) {
		if self.queueGeneration == generation {
			playerLog("Queue generation (\(generation)) gets prepended with \(audioTracks.count) tracks.")
			self.queue.prepend(audioTracks)
			self.callPlayStateChangeCallbacks()
		} else {
			playerLog("Queue generation (\(generation)) isn't the same as the current (\(self.queueGeneration)). Won't prepend \(audioTracks.count) tracks.")
		}
	}

	open func append(_ audioTracks: [AudioTrack], toQueue generation: Int) {
		if self.queueGeneration == generation {
			playerLog("Queue generation (\(generation)) gets appended with \(audioTracks.count) tracks.")
			self.queue.append(audioTracks)
			self.callPlayStateChangeCallbacks()
		} else {
			playerLog("Queue generation (\(generation)) isn't the same as the current (\(self.queueGeneration)). Won't append \(audioTracks.count) tracks.")
		}
	}
