...
open class AudioTrack: NSObject {
	...
	open func initNowPlayingInfo() {
        ...
        if let info = self as? AudioURLTrack {
            self.nowPlayingInfo?.merge((info.audioInfo?.metadata ?? [:]), uniquingKeysWith: { (_, new) -> NSObject in
                return new
            })
        }
	...
	open func durationInSeconds() -> Float? {
		guard let playerItem = self.playerItem, playerItem.duration != kCMTimeIndefinite else {
			return nil
		}
